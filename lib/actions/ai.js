...
- Do not add explanations or commentary`,
    ...
    const response = await callOpenRouter('/chat/completions', {
      ...
      messages,
      temperature: 0.3,
      maxTokens: 6144, // Token limit for single field translation (tripled)
    })
...
Maintain bilingual context where applicable.`
    ...
    const response = await callOpenRouter('/chat/completions', {
      ...
      maxTokens: 12288, // Token limit for content generation (tripled)
      messages: [
...
Use appropriate tone for: ${data.tone || 'professional'} audience.`
    ...
    const response = await callOpenRouter('/chat/completions', {
      model,
      response_format: { type: 'json_object' },
      maxTokens: 12288, // Token limit for article generation (tripled)
      messages: [
        { role: 'system', content: systemPrompt },
...
- Do not add explanations or commentary`,
    ...
    const response = await callOpenRouter('/chat/completions', {
      ...
      messages,
      temperature: 0.3,
      maxTokens: 12288, // Token limit for batch translations (tripled)
    })
...
Use appropriate tone for: ${data.tone || 'professional'} audience.`
    ...
    const response = await callOpenRouter('/chat/completions', {
      model,
      response_format: { type: 'json_object' },
      maxTokens: 12288, // Token limit for project generation (tripled)
      messages: [
        { role: 'system', content: systemPrompt },
...
Instructions: ${instructions || 'Improve clarity, grammar, and readability.'}`
    ...
    const response = await callOpenRouter('/chat/completions', {
      model,
      maxTokens: 6144, // Token limit for content refinement (tripled)
      messages: [
        { role: 'system', content: systemPrompt },
