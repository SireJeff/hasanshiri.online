...
export async function aiGenerateSEO(data) {
  ...
  try {
    ...
    const userPresets = presetsResult.presets || {}
    // Use articleGenerator preset for SEO (or contentRefiner as fallback)
    const model = userPresets.articleGenerator || userPresets.contentRefiner

    if (!model) {
      return { error: 'No preset configured for SEO generation. Please set Article Generator preset in AI Settings.', success: false }
    }
...
- Never leave any field empty - always provide all 4 fields`
    ...
    try {
      ...
      console.error('Failed to parse SEO AI response:', e)
      return { error: 'Failed to parse generated SEO content', success: false }
    }
