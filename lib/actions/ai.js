...
export async function aiGenerateProject(data) {
  ...
  try {
    // Support both 'prompt' and 'description' for flexibility
    ...
    const systemPrompt = `You are a JSON API that generates project documentation. You MUST respond with ONLY valid JSON - no explanations, no markdown, no conversational text.

Generate project documentation based STRICTLY on the user's input.

CRITICAL RULES:
1. If the user provides a project NAME, use it EXACTLY as the title_en (do not rename or rephrase it)
2. If the user provides a project DESCRIPTION, use it as the basis for description_en (you may polish grammar/clarity but keep the meaning)
3. For long_description_en, expand on the user's input with relevant sections - do not invent unrelated content
4. Maintain the scope and context of what the user provides - don't add features or technologies not mentioned
5. Keep the technical accuracy - if user mentions specific tech (React, Python, etc.), preserve it

Respond with ONLY this JSON structure (no other text):
{
  "title_en": "The project name from user input (exact match)",
  "description_en": "A brief 2-3 sentence description based on user's input",
  "long_description_en": "Detailed description with HTML formatting (<h3>, <p>, <ul>, <li> tags) including sections: Overview, Technologies, Features, Results"
}
...
Tone: ${data.tone || 'professional'}`
    ...
    try {
      ...
      console.error('Failed to parse AI response:', e, 'Raw result:', result?.substring(0, 500))
      return { error: 'Failed to parse generated content. The AI returned an unexpected format.', success: false, debug: result?.substring(0, 200) }
    }
