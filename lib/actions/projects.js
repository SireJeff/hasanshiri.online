...
export async function getAllProjectSlugs() {
  ...
  return projects?.map(p => ({ slug: p.slug, updated_at: p.updated_at })) || []
}
...
export async function updateProject(id, formData) {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  console.log('[updateProject] User authenticated:', !!user)

  if (!user) {
    console.log('[updateProject] ERROR: Not authenticated')
    return { error: 'Not authenticated' }
  }
  ...
  console.log('[updateProject] Updating project with data:', { id, title: formData.title_en, tagCount: tagIds.length })
  ...
  if (error) {
    console.error('[updateProject] Supabase update error:', error)
    return { error: error.message }
  }

  console.log('[updateProject] Project updated successfully:', project?.id)
  ...
  if (tagIds.length > 0) {
    ...
    const { error: tagError } = await supabase.from('project_tag_relations').insert(tagInserts)
    if (tagError) {
      console.error('[updateProject] Error updating tags:', tagError)
    } else {
      console.log('[updateProject] Tags updated:', tagIds.length)
    }
  }
  ...
  console.log('[updateProject] Complete! Returning project.')
  return { project }
}
